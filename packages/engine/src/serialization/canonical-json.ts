import { deepFreeze } from "../state/freeze";

export const CANONICAL_JSON_VERSION = 1 as const;
export const CANONICAL_HASH_ALGORITHM = "sha256" as const;

export type CanonicalHashV1 = `sha256:${string}`;

export class CanonicalSerializationError extends Error {
  readonly code: string;

  constructor(code: string, message: string) {
    super(`${code}: ${message}`);
    this.name = "CanonicalSerializationError";
    this.code = code;
  }
}

function reject(code: string, message: string): never {
  throw new CanonicalSerializationError(code, message);
}

function encodeCanonical(value: unknown, path: string, ancestors: Set<object>): string {
  if (value === null) return "null";
  if (typeof value === "boolean") return value ? "true" : "false";
  if (typeof value === "string") return JSON.stringify(value);
  if (typeof value === "number") {
    if (!Number.isSafeInteger(value)) {
      reject("CANONICAL_NUMBER_INVALID", `${path} must be a finite safe integer.`);
    }
    return String(value);
  }
  if (typeof value !== "object") {
    reject("CANONICAL_VALUE_INVALID", `${path} contains unsupported ${typeof value}.`);
  }
  if (ancestors.has(value)) reject("CANONICAL_CYCLE", `${path} contains a cycle.`);
  ancestors.add(value);
  try {
    if (Array.isArray(value)) {
      if (Object.getOwnPropertySymbols(value).length > 0) {
        reject("CANONICAL_KEY_INVALID", `${path} must not contain symbol keys.`);
      }
      const allowedKeys = new Set([
        "length",
        ...Array.from({ length: value.length }, (_entry, index) => String(index)),
      ]);
      if (Object.getOwnPropertyNames(value).some((key) => !allowedKeys.has(key))) {
        reject("CANONICAL_ARRAY_INVALID", `${path} must not contain holes or extra properties.`);
      }
      const encoded: string[] = [];
      for (let index = 0; index < value.length; index += 1) {
        const descriptor = Object.getOwnPropertyDescriptor(value, String(index));
        if (descriptor === undefined || !descriptor.enumerable || !("value" in descriptor)) {
          reject(
            "CANONICAL_ARRAY_INVALID",
            `${path}[${index}] must be an enumerable data element.`,
          );
        }
        encoded.push(encodeCanonical(descriptor.value, `${path}[${index}]`, ancestors));
      }
      return `[${encoded.join(",")}]`;
    }
    const prototype = Object.getPrototypeOf(value);
    if (prototype !== Object.prototype && prototype !== null) {
      reject("CANONICAL_OBJECT_INVALID", `${path} must be a plain record.`);
    }
    if (Object.getOwnPropertySymbols(value).length > 0) {
      reject("CANONICAL_KEY_INVALID", `${path} must not contain symbol keys.`);
    }
    const propertyNames = Object.getOwnPropertyNames(value);
    if (propertyNames.length !== Object.keys(value).length) {
      reject("CANONICAL_KEY_INVALID", `${path} must contain enumerable data properties only.`);
    }
    const record = value as Record<string, unknown>;
    return `{${propertyNames
      .sort()
      .map((key) => {
        const descriptor = Object.getOwnPropertyDescriptor(record, key);
        if (descriptor === undefined || !("value" in descriptor)) {
          reject("CANONICAL_PROPERTY_INVALID", `${path}.${key} must be a data property.`);
        }
        return `${JSON.stringify(key)}:${encodeCanonical(descriptor.value, `${path}.${key}`, ancestors)}`;
      })
      .join(",")}}`;
  } finally {
    ancestors.delete(value);
  }
}

export function canonicalStringifyV1(value: unknown): string {
  return encodeCanonical(value, "$", new Set());
}

function utf8Bytes(value: string): Uint8Array {
  const bytes: number[] = [];
  for (let index = 0; index < value.length; index += 1) {
    const first = value.charCodeAt(index);
    let point = first;
    if (first >= 0xd800 && first <= 0xdbff && index + 1 < value.length) {
      const second = value.charCodeAt(index + 1);
      if (second >= 0xdc00 && second <= 0xdfff) {
        point = 0x10000 + ((first - 0xd800) << 10) + (second - 0xdc00);
        index += 1;
      }
    }
    if (point <= 0x7f) {
      bytes.push(point);
    } else if (point <= 0x7ff) {
      bytes.push(0xc0 | (point >>> 6), 0x80 | (point & 0x3f));
    } else if (point <= 0xffff) {
      bytes.push(0xe0 | (point >>> 12), 0x80 | ((point >>> 6) & 0x3f), 0x80 | (point & 0x3f));
    } else {
      bytes.push(
        0xf0 | (point >>> 18),
        0x80 | ((point >>> 12) & 0x3f),
        0x80 | ((point >>> 6) & 0x3f),
        0x80 | (point & 0x3f),
      );
    }
  }
  return Uint8Array.from(bytes);
}

const SHA256_INITIAL = [
  0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19,
] as const;

const SHA256_CONSTANTS = [
  0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
  0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
  0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
  0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
  0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
  0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
  0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
  0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
] as const;

function rotateRight(value: number, count: number): number {
  return (value >>> count) | (value << (32 - count));
}

export function sha256Hex(value: string): string {
  const source = utf8Bytes(value);
  const paddedLength = Math.ceil((source.length + 9) / 64) * 64;
  const message = new Uint8Array(paddedLength);
  message.set(source);
  message[source.length] = 0x80;
  const bitLength = source.length * 8;
  const high = Math.floor(bitLength / 0x1_0000_0000);
  const low = bitLength >>> 0;
  for (let offset = 0; offset < 4; offset += 1) {
    message[paddedLength - 8 + offset] = (high >>> (24 - offset * 8)) & 0xff;
    message[paddedLength - 4 + offset] = (low >>> (24 - offset * 8)) & 0xff;
  }

  const hash: number[] = [...SHA256_INITIAL];
  const words = new Uint32Array(64);
  for (let chunk = 0; chunk < message.length; chunk += 64) {
    for (let index = 0; index < 16; index += 1) {
      const offset = chunk + index * 4;
      words[index] =
        (((message[offset] ?? 0) << 24) |
          ((message[offset + 1] ?? 0) << 16) |
          ((message[offset + 2] ?? 0) << 8) |
          (message[offset + 3] ?? 0)) >>>
        0;
    }
    for (let index = 16; index < 64; index += 1) {
      const before15 = words[index - 15] ?? 0;
      const before2 = words[index - 2] ?? 0;
      const sigma0 = rotateRight(before15, 7) ^ rotateRight(before15, 18) ^ (before15 >>> 3);
      const sigma1 = rotateRight(before2, 17) ^ rotateRight(before2, 19) ^ (before2 >>> 10);
      words[index] = ((words[index - 16] ?? 0) + sigma0 + (words[index - 7] ?? 0) + sigma1) >>> 0;
    }

    let [a, b, c, d, e, f, g, h] = hash as [
      number,
      number,
      number,
      number,
      number,
      number,
      number,
      number,
    ];
    for (let index = 0; index < 64; index += 1) {
      const sum1 = rotateRight(e, 6) ^ rotateRight(e, 11) ^ rotateRight(e, 25);
      const choice = (e & f) ^ (~e & g);
      const temporary1 =
        (h + sum1 + choice + (SHA256_CONSTANTS[index] ?? 0) + (words[index] ?? 0)) >>> 0;
      const sum0 = rotateRight(a, 2) ^ rotateRight(a, 13) ^ rotateRight(a, 22);
      const majority = (a & b) ^ (a & c) ^ (b & c);
      const temporary2 = (sum0 + majority) >>> 0;
      h = g;
      g = f;
      f = e;
      e = (d + temporary1) >>> 0;
      d = c;
      c = b;
      b = a;
      a = (temporary1 + temporary2) >>> 0;
    }
    hash[0] = ((hash[0] ?? 0) + a) >>> 0;
    hash[1] = ((hash[1] ?? 0) + b) >>> 0;
    hash[2] = ((hash[2] ?? 0) + c) >>> 0;
    hash[3] = ((hash[3] ?? 0) + d) >>> 0;
    hash[4] = ((hash[4] ?? 0) + e) >>> 0;
    hash[5] = ((hash[5] ?? 0) + f) >>> 0;
    hash[6] = ((hash[6] ?? 0) + g) >>> 0;
    hash[7] = ((hash[7] ?? 0) + h) >>> 0;
  }
  return hash.map((word) => word.toString(16).padStart(8, "0")).join("");
}

export function hashCanonicalV1(value: unknown): CanonicalHashV1 {
  return `sha256:${sha256Hex(canonicalStringifyV1(value))}`;
}

export function canonicalDescriptorV1(value: unknown): Readonly<{
  canonicalizationVersion: typeof CANONICAL_JSON_VERSION;
  algorithm: typeof CANONICAL_HASH_ALGORITHM;
  hash: CanonicalHashV1;
}> {
  return deepFreeze({
    canonicalizationVersion: CANONICAL_JSON_VERSION,
    algorithm: CANONICAL_HASH_ALGORITHM,
    hash: hashCanonicalV1(value),
  });
}
